...
복사 및 동기화는 원본과 대상 간의 데이터 동기화를 시작합니다. 동기화에 소요된 시간, 중단 여부, 복사, 스캔 또는 삭제된 파일 수에 대한 동기화 통계를 확인할 수 있습니다. 그런 다음, link:task-managing-relationships.html["동기화 관계"], link:task-managing-data-brokers.html["데이터 브로커를 관리하세요"], 또는 link:task-managing-reports.html["성능과 구성을 최적화하기 위한 보고서 생성"]을 관리할 수 있습니다.
