...
+
Red Hat Enterprise Linux 시스템은 Red Hat Subscription Management에 등록되어야 합니다. 등록되지 않은 경우 시스템은 설치 중에 필요한 타사 소프트웨어를 업데이트하기 위한 리포지토리에 액세스할 수 없습니다.
...
. *새 동기화 만들기*를 선택합니다.
. *동기화 관계 정의* 페이지에서 소스와 대상을 선택한 다음 *계속*을 선택합니다.
+
*데이터 브로커 그룹* 페이지에 도달할 때까지 단계를 완료하세요.

. *Data Broker Group* 페이지에서 *Create Data Broker*를 선택한 다음 *On-Prem Data Broker*를 선택합니다.
+
...
. 데이터 브로커의 이름을 입력한 다음 *Continue*를 선택합니다.
+
안내 페이지가 로드되면 설치 프로그램을 다운로드할 수 있는 고유 링크가 포함된 안내를 따르십시오.

. 지침 페이지에서:
+
.. *AWS*, *Google Cloud* 또는 둘 다에 대한 액세스를 활성화하도록 선택합니다.
